...
class plgSystemJDBuilder extends JPlugin
{
   ...
   public function onBeforeRender()
   {
      ...
      if ($this->app->isAdmin() && ($this->isPageEdit() || $this->isArticleEdit() || $this->isModuleEdit())) {
   ...
   public function onAfterRender()
   {
      ...
      if ($this->isArticleListing()) {
         $JDBArticles = AuditHelper::getJDBArticles(true);
         $article_layouts = [];
         foreach ($JDBArticles as $item) {
            $article_layouts[] = $item->id;
         }
         self::$article_layouts = $article_layouts;
   ...
   public function isArticleListing()
   {
      ...
      return ($option == "com_content" && ($view == "articles" || $view == "") && $layout == "" && $article_integration);
   }
   ...
   public function isArticleEditing()
   {
      ...
      return ($option == "com_content" && $task == "article.edit" && $article_integration);
   }
   ...
   public function isArticleEdit()
   {
      ...
      $view = $this->app->input->get('view', '');
      $layout = $this->app->input->get('layout', '');
      $status = ($option == "com_content" && $view == "article" && $layout == "edit" && $article_integration);

      if ($status) {
         $articleComponentConfig = JComponentHelper::getParams('com_content');
         $articleComponentConfig = $articleComponentConfig->get('show_article_options', 1, 'INT');
         if (!$articleComponentConfig) {
            JFactory::getApplication()->enqueueMessage(JText::_('JDB_ARTICLE_OPTIONS_ALERT'), 'warning');
            $status = false;
         }
      }
      return $status;
   ...
   {
      \JForm::addFormPath(JDBPATH_PLUGIN . '/forms');
   ...
   {
      $builder_params = JComponentHelper::getParams('com_jdbuilder');
      $article_integration = $builder_params->get('article_integration',  1);
      $option = $this->app->input->get('option');
      $view = $this->app->input->get('view');
      $task = $this->app->input->get('task');

      if ((($option == 'com_content' && $view == 'article' && $context === 'com_content.article')) && $this->app->isSite() && $article_integration) {
         $para = $params;
         $enabled = (int) $para->get('jdbuilder_layout_enabled', 0);
         $layout_id = (int) $para->get('jdbuilder_layout_id', 0);
         if ($enabled && !empty($layout_id)) {
            $layout = \JDPageBuilder\Helper::getLayout($layout_id);
            $layout = new \JDPageBuilder\Element\Layout($layout, 'article', $article->id);
